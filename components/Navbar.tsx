    ...
    <nav className="sticky top-0 z-50 w-full glass-panel border-b border-slate-200/60 shadow-sm transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-28 sm:h-32">
          {/* Logo avec nouvelles couleurs */}
          <div 
            className="flex items-center gap-3 cursor-pointer group flex-shrink-0 mr-6" 
            onClick={() => onTabChange('normal')}
          >
            <div className="bg-emerald-500 text-white p-3 rounded-xl shadow-emerald-200 shadow-lg group-hover:scale-105 transition-transform">
              <Stethoscope size={32} strokeWidth={3} />
            </div>
            <span className="text-3xl sm:text-4xl font-black tracking-tighter whitespace-nowrap">
              <span className="text-blue-600">Dicta</span><span className="text-emerald-600">Med</span>
            </span>
          </div>
          
          <div className="flex items-center gap-4 flex-1 justify-end">
            <div className="flex bg-slate-100/80 p-2.5 rounded-2xl overflow-x-auto no-scrollbar max-w-[calc(100vw-220px)] sm:max-w-none">
              {navItems.map((item) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex items-center px-8 py-5 rounded-xl text-lg font-black transition-all duration-200 whitespace-nowrap
                      ${isActive 
                    ...
                    `}
                  >
                    <Icon size={24} className={`sm:mr-3 ${isActive ? 'text-emerald-500' : 'text-slate-400'}`} strokeWidth={2.5} />
            ...
            {user && (
              <div className="hidden md:flex items-center gap-4 border-l border-slate-200 pl-6 flex-shrink-0">
                ...
                <button 
                  onClick={onLogout}
                  className="p-4 rounded-xl bg-slate-50 text-slate-400 hover:text-rose-600 hover:bg-rose-50 border border-slate-200 transition-all hover:shadow-md"
                  title="Déconnexion"
                >
